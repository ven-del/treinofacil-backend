...
const treinoController = {
        ...
        try {
            ...
            if (treino.plano_id) {
                ...
                if (!plano || plano.aluno_id !== alunoId) {
                    return res.status(403).json({ message: "Acesso negado: Este treino não pertence aos seus planos." });
                }
            }            
            const exerciciosDoTreino = await treinoExercicioModel.getExerciciosDoTreino(treinoId);

            const detalhesCompletosDosExercicios = await Promise.all(exerciciosDoTreino.map(async (te) => {
                const exercicioDetalhes = await exercicioModel.getExercicioById(te.exercicio_id);
                return {
                    ...te,
            ...
            console.error(`Erro ao obter detalhes do treino ${treinoId} para o aluno ${alunoId}:`, error.message);
    ...
    async getExerciciosByGrupo(req, res) {
        ...
        try {
            let exercicios;
            if (grupoMuscular) {
                exercicios = await exercicioModel.getExerciciosByGrupoMuscular(grupoMuscular);
            } else {
                exercicios = await exercicioModel.getAllExercicios(); // Assumindo que este método exista no exercicioModel
            }

            if (!exercicios || exercicios.length === 0) {
                return res.status(200).json({ message: "Nenhum exercício encontrado.", exercicios: [] });
            }
            ...
            console.error(`Erro ao obter exercícios para o aluno ${alunoId} (grupo: ${grupoMuscular || 'todos'}):`, error.message);
            return res.status(500).json({
    ...
    async createTreino(req, res) {
        ...
        try {
            const newTreino = await treinoModel.createTreino({ nome, descricao, plano_id });

            if (exercicios && exercicios.length > 0) {
                const exerciciosAdicionados = await Promise.all(exercicios.map(async (ex) => {
                    const existingExercicio = await exercicioModel.getExercicioById(ex.id);
                    if (!existingExercicio) {
                        console.warn(`Exercício com ID ${ex.id} não encontrado. Será ignorado ou tratado como erro.`);
