...
const calendarioTreinoModel = {
  async createEvento(eventoData) {
    if (
      ...
      !eventoData.aluno_id ||
      !eventoData.data ||
      !eventoData.treino_id
    ) {
      throw new Error(
        "Dados de evento inválidos: aluno_id, data e treino_id são obrigatórios."
      );
    }
    ...
    if (error) {
      console.error(
        "Erro ao criar evento no calendário de treino:",
        error.message
      );
      throw new Error(
        `Não foi possível criar o evento no calendário: ${error.message}`
      );
    }
    return data;
  },
  ...
  async getEventosByAlunoAndDate(alunoId, date) {
    if (!alunoId || !date) {
      throw new Error(
        "ID do aluno e data são obrigatórios para buscar eventos."
      );
    }
    const { data, error } = await supabase
      .from(CALENDARIO_TREINO_TABLE)
    ...
    if (error) {
      ...
      throw new Error(
        `Não foi possível buscar eventos para a data: ${error.message}`
      );
    }
    return data;
  },
  ...
  async getEventosByAlunoAndDateRange(alunoId, startDate, endDate) {
    if (!alunoId || !startDate || !endDate) {
      throw new Error(
        "ID do aluno, data de início e data de fim são obrigatórios para buscar eventos por período."
      );
    }
    const { data, error } = await supabase
      .from(CALENDARIO_TREINO_TABLE)
    ...
    if (error) {
      ...
      throw new Error(
        `Não foi possível buscar eventos para o período: ${error.message}`
      );
    }
    return data;
  },
  ...
  async updateEventoStatus(eventoId, newStatus) {
    if (!eventoId || !newStatus) {
      throw new Error(
        "ID do evento e novo status são obrigatórios para atualização."
      );
    }
    ...
    if (error) {
      console.error(
        "Erro ao atualizar status do evento de treino:",
        error.message
      );
      throw new Error(
        `Não foi possível atualizar o status do evento: ${error.message}`
      );
    }
    return data;
  },
  ...
  async deleteEvento(eventoId) {
    ...
    if (error) {
      console.error(
        "Erro ao deletar evento do calendário de treino:",
        error.message
      );
