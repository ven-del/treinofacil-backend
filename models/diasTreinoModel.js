...
const diasTreinoModel = {
  async addDiaTreino(treinoId, diaSemana) {
    if (!treinoId || !diaSemana) {
      throw new Error(
        "ID do treino e dia da semana são obrigatórios para adicionar dia de treino."
      );
    }
    ...
    if (!allowedDays.includes(normalizedDiaSemana)) {
      throw new Error(
        `Dia da semana inválido: '${diaSemana}'. Dias permitidos são: ${allowedDays.join(
          ", "
        )}.`
      );
    }
    ...
    if (error) {
      ...
      throw new Error(
        `Não foi possível adicionar o dia de treino: ${error.message}`
      );
    }
    return data;
  },
  ...
  async getDiasTreinoByTreinoId(treinoId) {
    ...
    if (error) {
      ...
      throw new Error(
        `Não foi possível buscar dias de treino para o treino: ${error.message}`
      );
    }
    return data;
  },
  ...
  async getDiaTreinoByTreinoAndDay(treinoId, diaSemana) {
    if (!treinoId || !diaSemana) {
      throw new Error(
        "ID do treino e dia da semana são obrigatórios para busca."
      );
    }
    const { data, error } = await supabase
      .from(DIAS_TREINO_TABLE)
    ...
    if (error && error.code !== "PGRST116") {
      console.error(
        "Erro ao buscar dia de treino por treino e dia da semana:",
        error.message
      );
  ...
  async removeDiaTreino(diaTreinoId) {
    ...
    if (error) {
      ...
      throw new Error(
        `Não foi possível remover o dia de treino: ${error.message}`
      );
    }
    return count > 0;
  },
  ...
  async removeAllDiasTreino(treinoId) {
    if (!treinoId) {
      throw new Error(
        "ID do treino é obrigatório para remover todos os dias de treino."
      );
    }
    const { error, count } = await supabase
      .from(DIAS_TREINO_TABLE)
    ...
    if (error) {
      console.error(
        "Erro ao remover todos os dias de treino do treino:",
        error.message
      );
      throw new Error(
        `Não foi possível remover todos os dias de treino: ${error.message}`
      );
    }
    return count > 0;
  },
